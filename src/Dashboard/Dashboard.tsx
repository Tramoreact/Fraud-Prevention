...
//         <Button variant="contained" onClick={hanldeClick}>
//           Blocked List
//         </Button>
...
const Dashboard = () => {
  ...
  const handleSubmitBlock = async () => {
    try {
      ...
      const response = await axios.post(`${Config.API_URL}/suspicious`, form, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        fetchData();
        setOpenSecondDialog(false);
        setFormData({ type: "", value: "", remarks: "", attachment: null });
        setSuccessDialog(true);
      }
    } catch (error: any) {
      console.error("Error:", error.response?.data || error.message);
          ...
          <Table size="medium">
            <TableHead>
              <TableRow>
                ...
                <TableCell>Risk Level</TableCell>
                <TableCell>Remarks</TableCell>
                <TableCell>Triggered Values</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {paginatedData.map((row: any, idx: number) => {
